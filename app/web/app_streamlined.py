...
class StockScanner:
    ...
    def start_background_scanner(self) -> None:
        ...
        self.scanner_thread = threading.Thread(target=background_scan_loop, daemon=True)
...
def api_scanner_status():
    ...
    try:
        is_running = scanner.scanner_thread and scanner.scanner_thread.is_alive()
        stock_count = len(scanner.cache['stocks']) if scanner.cache['stocks'] else 0
        ...
        return jsonify({
            ...
            'last_scan': scanner.cache.get('last_update_str', 'Never') if scanner.cache else 'Never'
        })
    except Exception as e:
        return jsonify({
            'error': str(e),
...
def initialize_app():
    ...
    # Initialize traffic analytics
    try:
        traffic_analytics.init_database()
        logger.info("✅ Traffic analytics database initialized")
    except Exception as e:
    ...
    try:
        scanner.load_cache()
        logger.info("✅ Cache loaded with {} stocks".format(len(scanner.cache['stocks'])))
    except Exception as e:
