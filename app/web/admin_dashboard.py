...
class TrafficAnalytics:
        ...
        try:
            ...
            cursor.execute('''
                CREATE TABLE IF NOT EXISTS visitors (
                    ...
                    is_return_visitor BOOLEAN DEFAULT 0,
    ...
    def track_visitor(self, request):
        ...
        try:
            ...
            with self.lock:
                ...
                # Check if visitor exists by IP address (for return visitor detection)
                cursor.execute('''
                    SELECT id, visit_count, first_visit, is_return_visitor, return_visits 
                    FROM visitors 
                    WHERE ip_address = ?
                ''', (ip_address,))
                ...
                if result:
                    ...
                    visitor_id, visit_count, first_visit, is_return_visitor, return_visits = result
                    new_visit_count = visit_count + 1
                    
                    # Determine if this is a return visit (more than 1 visit)
                    new_is_return_visitor = new_visit_count > 1
                    new_return_visits = return_visits + 1 if new_is_return_visitor else return_visits
                    
                    cursor.execute('''
                        UPDATE visitors 
                        SET last_visit = ?, visit_count = ?, is_return_visitor = ?, return_visits = ?
                        WHERE id = ?
                    ''', (current_time, new_visit_count, new_is_return_visitor, new_return_visits, visitor_id))
                else:
                    # Create new visitor
                    cursor.execute('''
                        INSERT INTO visitors (session_id, ip_address, user_agent, first_visit, last_visit, 
                                            visit_count, is_return_visitor, return_visits)
                        VALUES (?, ?, ?, ?, ?, 1, 0, 0)
    ...
    def get_traffic_stats(self, days=7):
        ...
        try:
            ...
            # Return visitor statistics - excluding local IPs
            cursor.execute(f'''
                SELECT COUNT(*) FROM visitors 
                WHERE is_return_visitor = 1 
                AND first_visit >= ? 
                AND ip_address NOT IN ({excluded_ips_placeholders})
            ''', [start_date] + excluded_ips)
            return_visitors = cursor.fetchone()[0]
            
            # Total return visits - excluding local IPs
            cursor.execute(f'''
                SELECT SUM(return_visits) FROM visitors 
                WHERE first_visit >= ? 
                AND ip_address NOT IN ({excluded_ips_placeholders})
            ''', [start_date] + excluded_ips)
            total_return_visits = cursor.fetchone()[0] or 0
            
            # New vs return visitors today - excluding local IPs
            cursor.execute(f'''
                SELECT 
                    COUNT(CASE WHEN is_return_visitor = 0 THEN 1 END) as new_visitors,
                    COUNT(CASE WHEN is_return_visitor = 1 THEN 1 END) as return_visitors_today
                FROM visitors 
                WHERE DATE(last_visit) = ? 
                AND ip_address NOT IN ({excluded_ips_placeholders})
            ''', [today] + excluded_ips)
            visitor_types = cursor.fetchone()
            new_visitors_today = visitor_types[0] or 0
            return_visitors_today = visitor_types[1] or 0
            
            # Top return visitors (by visit count) - excluding local IPs
            cursor.execute(f'''
                SELECT ip_address, visit_count, return_visits, first_visit, last_visit
                FROM visitors 
                WHERE is_return_visitor = 1 
                AND first_visit >= ? 
                AND ip_address NOT IN ({excluded_ips_placeholders})
                ORDER BY visit_count DESC, return_visits DESC
                LIMIT 10
            ''', [start_date] + excluded_ips)
            top_return_visitors = cursor.fetchall()
            ...
            return {
                ...
                'return_visitors_today': return_visitors_today,
                'top_return_visitors': top_return_visitors
            }
